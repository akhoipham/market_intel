...
.tapesort{background:none;border:0;color:var(--dim);font:600 .62rem var(--mono);
  letter-spacing:.14em;cursor:pointer;padding:0;border-bottom:1px dashed var(--faint)}
.tapesort:hover{color:var(--amber);border-bottom-color:var(--amber-dim)}
...
<div class="tape-label">THEME TAPE — <button class="tapesort" id="tape-sort" title="Click to change theme order">MENTIONS ↓</button> <span id="active-filters"></span></div>
...
<div class="grid">
  ...
  <section class="pane">
    <h2>Headline Tape <span id="hl-count" style="color:var(--faint)"></span>
      <span class="feedtabs" id="brackets" role="tablist" aria-label="Headline bracket">
        <button data-br="equities" class="on" role="tab">Equities</button>
        <button data-br="macro" role="tab">Macro</button>
        <button data-br="insider" role="tab">Insider</button>
      </span>
    </h2>
...
const EXCLUDED_TICKERS = new Set(["NWS", "NWSA", "TISI", "BLSH"]);

const S = {range:86400, fromDate:null, toDate:null, theme:null, ticker:null, q:"",
           sortK:"n", sortDir:-1, cap:"", sent:null, bracket:"equities", tapeSort:"n"};

const isInsider = a => a.k==="filing" && /^Form 4/.test(a.t);
// Top-level bracket. Prefer the value baked in by the backend (themes.bracket_for);
// fall back to deriving it client-side so the page still works against a data.json
// built before this field existed (e.g. during the deploy window).
const bracketOf = a => a.br
  || (isInsider(a) ? "insider"
      : (a.ln||[]).some(l => l==="Macro & Rates" || l==="Geopolitics") ? "macro"
      : "equities");
...
function passLens(a){ return true; }   // lens dropdown removed; brackets supersede it
...
function passBracket(a){ return bracketOf(a)===S.bracket; }
function visible(){ return DATA.filter(a=>inWindow(a)&&passText(a)&&passLens(a)
  &&passCap(a)&&passSent(a)&&passBracket(a)
...
function render(){
  ...
  refreshAge();
  $("total-n").textContent = DATA.filter(a=>inWindow(a)&&passBracket(a)).length;

  const win = visible();
  const winNoTk = DATA.filter(a=>inWindow(a)&&passText(a)&&passLens(a)&&passCap(a)
    &&passSent(a)&&passBracket(a)&&(!S.theme||a.th.includes(S.theme)));
  // KPI counts are over the window (ignoring the active sentiment filter so the
  // tallies stay stable as you toggle them)
  const kbase = DATA.filter(a=>inWindow(a)&&passText(a)&&passLens(a)&&passCap(a)&&passBracket(a)
  ...
  const base = DATA.filter(a=>inWindow(a)&&passText(a)&&passLens(a)&&passCap(a)
    &&passSent(a)&&passBracket(a)&&(!S.ticker||a.tk.some(m=>m.ticker===S.ticker)));
  const tstat={};
  for(const a of base) for(const th of a.th){
    (tstat[th]||={n:0,s:0}); tstat[th].n++; tstat[th].s+=a.sn; }
  const tcmp = {
    n: (x,y)=> y[1].n - x[1].n,                       // most mentioned first
    s: (x,y)=> (y[1].s/y[1].n) - (x[1].s/x[1].n),     // most bullish net sentiment first
    a: (x,y)=> x[0].localeCompare(y[0]),              // A–Z by name
  }[S.tapeSort] || ((x,y)=>y[1].n-x[1].n);
  $("tape").innerHTML = Object.entries(tstat).sort(tcmp)
    .map(([name,v])=>{
...
$("brackets").addEventListener("click", e=>{
  const b=e.target.closest("button"); if(!b) return;
  S.bracket=b.dataset.br;
  document.querySelectorAll("#brackets button").forEach(x=>x.classList.toggle("on",x===b));
  render(); });
// theme-tape order toggle: mentions → net sentiment → A–Z → (repeat)
$("tape-sort").addEventListener("click", ()=>{
  const order=["n","s","a"], label={n:"MENTIONS ↓",s:"SENTIMENT ↓",a:"NAME A–Z"};
  S.tapeSort=order[(order.indexOf(S.tapeSort)+1)%order.length];
  $("tape-sort").textContent=label[S.tapeSort];
  render(); });
